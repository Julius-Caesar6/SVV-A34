...
def Tx(x):
    return Macaulay(x1, Ry1*-zhat, 0).result(x) + Macaulay(x2, Ry2*-zhat, 0).result(x) + Macaulay(x3,Ry3*-zhat,0).result(x) + Macaulay(x2-(xa/2), Rj*((ha/2)-zhat)*np.sin(beta),0).result(x) + Macaulay(x2-(xa/2),-Rj*np.cos(beta)*(ha/2),0).result(x) + Macaulay(x2+(xa/2), -P*np.cos(beta)*(ha/2), 0).result(x) + Macaulay(x2+(xa/2), P*np.sin(beta)*((ha/2)-zhat), 0).result(x) +Iswitch*IntegrateX(x,1,1)-zhat*Iswitch*IntegrateX(x,1,0)
...
def vz(x):
    return (-1/(E*Iyy)) * ( Macaulay(x1,-Ry1,3).result(x) + Macaulay(x2,-Ry2,3).result(x) + Macaulay(x3,-Ry3,3).result(x) + Macaulay(x2-0.5*xa,-Rj*np.sin(beta),3).result(x) + Macaulay(x2+0.5*xa,-P*np.sin(beta),3).result(x) - Iswitch*IntegrateX(x,4,0))

def theta(x):
    return (1/(G*J))*(  Macaulay(x1, Ry1*-zhat, 1).result(x) + Macaulay(x2, Ry2*-zhat, 1).result(x) + Macaulay(x3,Ry3*-zhat,1).result(x) + Macaulay(x2-(xa/2), Rj*((ha/2)-zhat)*np.sin(beta),1).result(x) + Macaulay(x2-(xa/2),-Rj*np.cos(beta)*(ha/2),1).result(x) + Macaulay(x2+(xa/2), -P*np.cos(beta)*(ha/2), 1).result(x) + Macaulay(x2+(xa/2), P*np.sin(beta)*((ha/2)-zhat), 1).result(x) +Iswitch*IntegrateX(x,2,1)-zhat*Iswitch*IntegrateX(x,2,0))

...
def Tx(x):
    return Macaulay(x1,Ry1*zhat,0).result(x) + Macaulay(x2,Ry2*zhat,0).result(x) + Macaulay(x3,Ry3*zhat,0).result(x) + scf*(  Macaulay(x2-0.5*xa,Rj*np.sin(beta)*zhat,0).result(x) + Macaulay(x2-0.5*xa,-Rj*np.cos(beta)*ha/2,0).result(x) + Macaulay(x2+0.5*xa,-P*np.cos(beta)*ha/2,0).result(x) + Macaulay(x2+0.5*xa,P*np.sin(beta)*(zhat),0).result(x) )  +Iswitch*IntegrateX(x,1,1)-zhat*Iswitch*IntegrateX(x,1,0)
...
def theta(x):
    return (1/(G*J))*(  Macaulay(x1,Ry1*zhat,1).result(x) +   Macaulay(x2,Ry2*zhat,1).result(x)  +   Macaulay(x3,Ry3*zhat,1).result(x) +   scv*(  Macaulay(x2-0.5*xa,Rj*np.sin(beta)*zhat,1).result(x)  + Macaulay(x2-0.5*xa,-Rj*np.cos(beta)*ha/2,1).result(x)   + Macaulay(x2+0.5*xa,P*np.sin(beta)*zhat,1).result(x)  + Macaulay(x2+0.5*xa,-P*np.cos(beta)*ha/2,1).result(x) ) + Iswitch*IntegrateX(x,2,1) - zhat*Iswitch*IntegrateX(x,2,0)       ) + C5

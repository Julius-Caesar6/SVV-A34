...
def Sy(x): #check
    return Macaulay(x1,-Ry1,0).result(x) + Macaulay(x2,-Ry2,0).result(x) + Macaulay(x3,-Ry3,0).result(x) + Macaulay(x2-0.5*xa,-Rj*np.sin(beta),0).result(x) + Macaulay(x2+0.5*xa,-P*np.sin(beta),0).result(x) -  Iswitch*IntegrateX(x,2,0)

def Sz(x): #check
    return Macaulay(x1,Rz1,0).result(x) + Macaulay(x2,Rz2,0).result(x) + Macaulay(x3,Rz3,0).result(x) + Macaulay(x2-0.5*xa,Rj*np.cos(beta),0).result(x) + Macaulay(x2+0.5*xa,P*np.cos(beta),0).result(x) + Iswitch*IntegrateX(x,2,0)

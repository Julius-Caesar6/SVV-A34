...
def reaction_solver(zhat, c, ha, d1, d2, d3, x1, x2, x3, xa, la, beta, P, E, Izz, Iyy, G, J): #TODO : ADJUST FOR UNITS!! (if needed?)
 ...
 #   [Rz1, Ry1, Rz2, Ry2, Rz3, Ry3, Rj, C1, C2, C3, C4, C5]
   ...
   # Second equation Mz:
    ...
    #b:
    brow6 = (-1/(E*Izz))*IntegrateX(x1,4,0) + d1*np.cos(beta)

    #   [Rz1, Ry1, Rz2, Ry2, Rz3, Ry3, Rj, C1, C2, C3, C4, C5]

    # Seventh equation Vy(x2..)
    row7 = [0, (Macaulay(x1, 1/(6*E*Izz), 3).result(x2)), 0, 0, 0, 0, (Macaulay(x2-(xa/2), np.sin(beta)/(6*E*Izz), 3).result(x2)), x2, 1, 0, 0, 0]
    #b:
    brow7 = (-1/(E*Izz))*IntegrateX(x2,4,0)

    # Eight equation Vy(x3..)
    row8 = [0, (Macaulay(x1, 1/(6*E*Izz), 3).result(x3)), 0, (Macaulay(x2, 1/(6*E*Izz), 3).result(x3)), 0, 0, (Macaulay(x2-(xa/2), np.sin(beta)/(6*E*Izz), 3).result(x3)), x3, 1, 0, 0, 0]
    #b:
    brow8 = Macaulay(x2+(xa/2), -P*np.sin(beta)/(6*E*Izz),3).result(x3) + (-1/(E*Izz))*IntegrateX(x3,4,0)  + d3*np.cos(beta)
    ...
    row12 = [Macaulay(x1, (np.cos(beta))/(-E*Iyy*6), 3).result(x2-(xa/2)), Macaulay(x1, np.sin(beta)/(6*E*Izz), 3).result(x2-(xa/2)) + Macaulay(x1, (-bcsf*ha*np.cos(beta)*zhat)/(2*G*J), 1).result(x2-(xa/2)) + Macaulay(x1, ((zhat**2)*np.sin(beta))/(G*J), 1).result(x2-(xa/2)), 0,0,0,0,0, np.sin(beta)*(x2-(xa/2)), np.sin(beta), np.cos(beta)*(x2-(xa/2)), np.cos(beta), 1*((((ha/2) - zhat)*np.sin(beta)) - bcsf*((ha/2)*np.cos(beta)))]
    #b:
    brow12 = (-np.sin(beta)/(E*Izz))*IntegrateX(x2-0.5*xa,4,0) - (zhat*((ha/2) - zhat)*np.sin(beta)/(G*J))*IntegrateX(x2-0.5*xa,2,1) + (np.sin(beta)*zhat*((ha/2) - zhat)/(G*J))*IntegrateX(x2-0.5*xa,2,0) + bcsf*(   (ha*np.cos(beta)/(2*G*J))*IntegrateX(x2-0.5*xa,2,1) - (ha*np.cos(beta)*zhat/(2*G*J))*IntegrateX(x2-0.5*xa,2,0)   )
...
xparam = (reaction_solver(zhat, Ca, ha, d1, d2, d3, x1, x2, x3, xa, la, beta, P, E, Izz, Iyy , G, J))

...
def reaction_solver(zhat, c, B, ha, d1, d2, d3, x1, x2, x3, xa, la, beta, P, E, Izz, Iyy, G, J): #TODO : ADJUST FOR UNITS!! (if needed?)
 ...
 #   [Rz1, Ry1, Rz2, Ry2, Rz3, Ry3, Rj, C1, C2, C3, C4, C5]
   ...
   # Second equation Mz:
    ...
    #b:
    brow2 = [Macaulay(x2+(xa/2),P*np.sin(beta), 1).result(la) + Integrate(la,2,0)]
    ...
    #b:
    brow3 = [Macaulay(x2+(xa/2), P*ha*np.cos(beta)/2, 0).result(la) - Macaulay(x2+(xa/2), P*np.sin(beta)*(zhat+(ha/2)), 0).result(la) - Integrate(la,1,1) + zhat*Integrate(la,1,0)]
    ...
    #b:
    brow4 = [Macaulay(x2+(xa/2), P*np.sin(beta),0).result(la)  - Integrate(la,1,0)]
    ...
    #b:
    brow6 = [(-1/(E*Izz))*Integrate(x1,4,0) + (zhat/(G*J))*Integrate(x1,2,1) - (zhat**2/(G*J))*Integrate(x1,2,0) + d1*np.cos(beta)]
    ...
    #b:
    brow7 = [(-1/(E*Izz))*Integrate(x2,4,0) +(zhat/(G*J))*Integrate(x2,2,1) - (zhat**2/(G*J))*Integrate(x2,2,0)]
    ...
    #b:
    brow8 = [P*(Macaulay(x2+(xa/2), np.sin(beta)/(-6*E*Izz),3).result(x3) + Macaulay(x2+(xa/2), (zhat*np.sin(beta)*(zhat+(ha/2)))/(G*J),1).result(x3) - Macaulay(x2+(xa/2), (zhat*np.cos(beta)*(ha/2))/(G*J), 1).result(x3)) + (-1/(E*Izz))*Integrate(x3,4,0)  + (zhat/(G*J))*Integrate(x3,2,1) - (zhat**2/(G*J))*Integrate(x3,2,0)  + d3*np.cos(beta)]
    ...
    #b:
    brow9 = [d3*np.sin(beta) + Macaulay(x2-(xa/2), P/(6*E*Iyy), 3).result(x3)]
    ...
    #b:
    brow10 = [Macaulay(x2-(xa/2), P/(6*E*Iyy),3).result(x2)]
    ...
    #b:
    brow12 = [(-np.sin(beta)/(E*Izz))*Integrate(x2-0.5*xa,4,0) - (zhat*np.sin(beta)/(G*J))*Integrate(x2-0.5*xa,2,1) + (np.sin(beta)*zhat**2/(G*J))*Integrate(x2-0.5*xa,2,0) + (ha*np.cos(beta)/(2*G*J))*Integrate(x2-0.5*xa,2,1) - (ha*np.cos(beta)*zhat/(2*G*J))*Integrate(x2-0.5*xa,2,0) ]

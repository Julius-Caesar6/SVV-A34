...
def reaction_solver(zhat, c, ha, d1, d2, d3, x1, x2, x3, xa, la, beta, P, E, Izz, Iyy, G, J): #TODO : ADJUST FOR UNITS!! (if needed?)
   ...
   # Second equation Mz:
    ...
    #b:
    brow3 = Macaulay(x2+(xa/2), P*ha*np.cos(beta)/2, 0).result(la) + Macaulay(x2+(xa/2), -P*np.sin(beta)*(zhat), 0).result(la) - IntegrateX(la,1,1) + zhat*IntegrateX(la,1,0)
    ...
    #b:
    brow8 = P*(   Macaulay(x2+(xa/2), np.sin(beta)/(6*E*Izz),3).result(x3) + Macaulay(x2+(xa/2), (zhat*np.cos(beta)*ha/(2*G*J)), 1).result(x3) + Macaulay(x2+(xa/2), (-zhat*np.sin(beta))/(G*J),1).result(x3)   ) + (-1/(E*Izz))*IntegrateX(x3,4,0)  + (zhat/(G*J))*IntegrateX(x3,2,1) - (zhat**2/(G*J))*IntegrateX(x3,2,0)  + d3*np.cos(beta)
    ...
    row12 = [Macaulay(x1, (np.cos(beta))/(-E*Iyy*6), 3).result(x2-(xa/2)), -1*Macaulay(x1, np.sin(beta)/(6*E*Izz), 3).result(x2-(xa/2)) + Macaulay(x1, (-bcsf*ha*np.cos(beta)*zhat)/(2*G*J), 1).result(x2-(xa/2)) + Macaulay(x1, ((zhat**2)*np.sin(beta))/(G*J), 1).result(x2-(xa/2)), 0,0,0,0,0, np.sin(beta)*(x2-(xa/2)), np.sin(beta), np.cos(beta)*(x2-(xa/2)), np.cos(beta), 1*((zhat*np.sin(beta)) - bcsf*((ha/2)*np.cos(beta)))]
    #b:
    brow12 = (-np.sin(beta)/(E*Izz))*IntegrateX(x2-0.5*xa,4,0) - (zhat*np.sin(beta)/(G*J))*IntegrateX(x2-0.5*xa,2,1) + (np.sin(beta)*zhat**2/(G*J))*IntegrateX(x2-0.5*xa,2,0) + bcsf*(   (ha*np.cos(beta)/(2*G*J))*IntegrateX(x2-0.5*xa,2,1) - (ha*np.cos(beta)*zhat/(2*G*J))*IntegrateX(x2-0.5*xa,2,0)   )
    ...
    #equations = np.array([row4, row5, row1, row2, row3, row6, row7, row8, row11, row10, row9, row12])
    #resultants = np.array([brow4, brow5, brow1, brow2, brow3, brow6, brow7, brow8, brow11, brow10, brow9, brow12])
...
xparam = (reaction_solver(-0.0053559, Ca, ha, d1, d2, d3, x1, x2, x3, xa, la, beta, P, E, 5.8159e-06, 4.3633e-05 , G, 8.62997e-06))

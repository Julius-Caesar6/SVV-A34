...
line1 = []
for i in output1:
    qb1_line = -1/Izz_total * ((t*h**2/4)*(-i) + A_stringer*y[1] + A_stringer*y[2])
    line1.append(qb1_line)

value1 = 1                   #integral (sin(theta).dtheta) from 0 to pi/2
qb1 = -1/Izz_total*(t*h**2/4*(-value1) + A_stringer*y[1] + A_stringer*y[2])
...
qb2 = -1/Izz_total *t* value2

plt.plot(input2,line2)
...
qb3 = qb1 + qb2 - 1/Izz_total*t*(h/2 - (h/2)/straight*value3+ A_stringer*sum(y[3:]))
...
qb4 = qb3 - 1/Izz_total*t *  ( ((-h/2)/straight)*value4 - A_stringer*sum(y[3:]))
...
qb5 = qb4 - 1/Izz_total*tspar *value5

plt.plot(input5,line5)
...
for e in output6:
    qb6_line =qb4 - qb5 - (1/Izz_total)*t*h**2/4 *(-e) - A_stringer*y[1]-A_stringer*y[2]
    line6.append(qb6_line)

value6 =      1               #integral (cos(theta)*dtheta) from -pi/2 to 0
qb6 = qb4 - qb5 - (1/Izz_total)*t*h**2/4 *(-e) - A_stringer*y[1]-A_stringer*y[2]

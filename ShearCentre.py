...
line1 = []
for i in output1:
    qb1_line = -1/Izz_total * ((t*h**2/4)*(-i +1) + A_stringer*y[1])
    line1.append(qb1_line)
#value1 = 1                   #integral (sin(theta).dtheta) from 0 to pi/2
qb1 = -1/Izz_total*(t*h**2/4*(-output1[-1] + 1 ) + A_stringer*y[1])
...
line3 = []
for k in output3:
    qb3_line = qb1 + qb2 - 1/Izz_total*t*(h/2 - (h/2)/straight *k + A_stringer*sum(y[2:]))
    line3.append(qb3_line)

#value3 =    827.44                #integral (y,dy) from 0 to straight
qb3 = qb1 + qb2 - 1/Izz_total*t*(h/2 - (h/2)/straight*output3[-1]+ A_stringer*sum(y[2:]))
...
line4 = []
for l in output4:
    qb4_line = qb3 - 1/Izz_total*t *  ( ((-h/2)/straight)*l - A_stringer*sum(y[2:]))
    line4.append(qb4_line)

#value4 =  827.44                  #integral (y*dy) from 0 to straight
qb4 = qb3 - 1/Izz_total*t *  ( ((-h/2)/straight)*output4[-1] - A_stringer*sum(y[2:]))
...
for e in output6:
    qb6_line =qb4 - qb5 -1/Izz_total*(t*h**2/4 *(-e+1) - A_stringer*y[1])
    line6.append(qb6_line)

#value6 =      1               #integral (cos(theta)*dtheta) from -pi/2 to 0
qb6 = qb4 - qb5 - 1/Izz_total*(t*h**2/4 *(-output6[-1] +1 ) - A_stringer*y[1])
